...
function LoginForm() {
  ...
  useEffect(() => {
    if (errorParam === 'banned') {
      setError('Sizning profilingiz taqiqlangan (Banned). Tizimga kirish imkonsiz!');
    }
  }, [errorParam]);
  ...
  return (
    <div className="w-full max-w-md p-8 rounded-3xl glass-premium glow-purple relative z-10 animate-fadeIn">
      <div className="flex flex-col items-center gap-3 text-center mb-8">
        ...
        <h2 className="text-2xl font-bold">{`Akkauntga Kirish`}</h2>
        <p className="text-xs text-[#94A3B8]">{`Darslarni boshlash uchun profilingizga kiring`}</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-5">
        <div className="flex flex-col gap-2">
          <label className="text-xs font-bold uppercase text-[#94A3B8] tracking-wider">Email</label>
      ...
      <p className="text-xs text-center text-[#94A3B8] mt-6">
        {`Profilingiz yo'qmi? `}
        <Link href={`/register?redirect=${encodeURIComponent(redirect)}`} className="text-[#D4AF37] font-semibold hover:underline">
          {`Ro'yxatdan o'tish`}
        </Link>
      </p>
    </div>
...
export default function LoginPage() {
  return (
    <div className="min-h-screen flex items-center justify-center px-6 py-12 relative select-none">
      ...
      <Suspense fallback={
        <div className="w-full max-w-md p-8 rounded-3xl glass text-center flex items-center justify-center min-h-[300px]">
